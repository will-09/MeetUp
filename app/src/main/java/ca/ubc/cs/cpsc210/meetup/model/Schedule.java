...
public class Schedule {
    ...
    public Set<String> getOneHourBreak(String dayOfWeek){
        SortedSet<Section> sections = getSectionsForDayOfWeek(dayOfWeek);
        if (sections.isEmpty()){
            return null;
        }
        ...
        if (sections.size() == 1) {
            Section section = sections.first();
            CourseTime courseTime = section.getCourseTime();
            startTimes.add(courseTime.getEndTime());   // works fine when having only one section
        } else if (sections.size() > 1) {
            Iterator<Section> it = sections.iterator();
            Section section = it.next(); // gives you the first section in the iterator
            String lastTime = section.getCourseTime().getEndTime(); // gives you the end time of the first section
            while (it.hasNext()) {
                section = it.next(); // next iterator
                String nextTime = section.getCourseTime().getStartTime(); //gives you the start time of the next iterator

                if (calculateBreakTimeInMinutes(nextTime, lastTime) >= 60) {
                    startTimes.add(lastTime);
                }
                lastTime = section.getCourseTime().getEndTime();
            }

        }

        startTimes.add(endTime(dayOfWeek).getEndTime());

        return startTimes;
    }

    public boolean containBreak (Set<String> st, String timeOfDay){

        if (st == null){
            return false;
        }

        int when = Integer.parseInt(timeOfDay);
        boolean contain = false;
        if (when == 10){
            if (st.contains("09:50")){
                contain = true;
            }
        ...
        if (when == 15){
            if (st.contains("14:50")) {
                contain = true;
            }

        }
        if (when == 18){
            if(st.contains("17:50")) {
                contain = true;
            }
...
/*
    public void getOneHourBreak(String dayOfWeek){
        Map<String, String> breakTime = new HashMap<String, String>();
        ...
        if (sections.size() == 1) {
            Section section = sections.first();
            CourseTime courseTime = section.getCourseTime();
            breakTime.put(courseTime.getEndTime(), "12:59");
            ...
            while (it.hasNext()) {
                ...
                if (calculateBreakTimeInMinutes(nextTime, lastTime) >= 60) {
                    breakTime.put(lastTime, nextTime);
                }
                lastTime = section.getCourseTime().getEndTime();
            }
        ...
        breakTime.put(endTime(dayOfWeek).getEndTime(), "12:59");
